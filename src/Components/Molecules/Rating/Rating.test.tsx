...
describe('Rating Component', () => {
    ...
    it('handles interaction when interactive', () => {
        ...
        render(<Rating rating={0} isInteractive={true} onRatingChange={handleChange} />);
        const stars = screen.getAllByRole('button');
    ...
    it('does not handle interaction when not interactive', () => {
        const handleChange = jest.fn();
        render(<Rating rating={0} isInteractive={false} onRatingChange={handleChange} />);
        const stars = screen.getAllByRole('button');
    ...
    it('applies custom className', () => {
        const { container } = render(<Rating rating={0} className="custom-class" />);
