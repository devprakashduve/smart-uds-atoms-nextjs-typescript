...
describe('FormHandler', () => {
  ...
  it('respects field order', () => {
    const props = {
      ...defaultProps,
      fieldOrder: ['checkbox', 'input'] as string[],
      inputFields: [{ id: 'i1', name: 'i1', label: 'Input 1', value: '', type: 'text' }],
    ...
    expect(checkbox.compareDocumentPosition(input)).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
  });

  it('renders nothing for unknown field type', () => {
     const props = {
      ...defaultProps,
      fieldOrder: ['unknown' as string],
    };
   ...
   it('uses default isRequired=true for textarea', () => {
     render(<FormHandler {...defaultProps} textAreaFields={[{ name: 'note', label: 'Note' }]} />);
   ...
   it('uses name as key if id is missing', () => {
      const props = {
        ...defaultProps,
        inputFields: [{ name: 'noIdInput', label: 'No ID', type: 'text', value: '' }],
        fieldOrder: ['input'] as string[],
      };
      render(<FormHandler {...props} />);
