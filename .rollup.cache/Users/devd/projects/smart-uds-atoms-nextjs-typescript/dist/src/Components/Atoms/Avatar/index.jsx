...
const Avatar = ({ src, alt, size = 16, className, width = 105, height = 105, circle = false, rounded = false, initials, status, }) => {
    ...
    return (<div className={containerClasses}>
      {showImage ? (<Img className={imgClasses} src={src} alt={alt} width={width} height={height} onError={() => setImageError(true)}/>) : (<div className="flex h-full w-full items-center justify-center bg-atom-avatar-background font-semibold text-atom-avatar-text" role="img" aria-label={alt}>
          {initials}
        </div>)}
