...
const Checkbox = ({ checked, label, toggleChecked, size = 'md', title, name, disabled = false, indeterminate = false, }) => {
    ...
    return (<div data-testid="checkmark-icon" className={`inline-flex items-center ${disabled ? 'cursor-not-allowed opacity-50' : ''}`}>
      <label className="relative flex cursor-pointer items-center">
        <input ref={inputRef} id={inputId} title={setTitle} type="checkbox" checked={isChecked} onChange={() => {
        ...
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform text-atom-input-text/60 opacity-0 peer-checked:opacity-100 peer-indeterminate:opacity-100">
          {indeterminate ? (<div className={`${boxSize} bg-atom-input-medium rounded-checkbox`}/>) : (<Icon name={'check'} variant={'outline'} className={`${boxSize} text-atom-input-text`}/>)}
        </span>
      </label>
      <label id={`${inputId}-label`} htmlFor={inputId} className={`ml-2 font-medium text-atom-input-text ${textSize} ${disabled ? 'cursor-not-allowed' : ''}`} onClick={(e) => disabled && e.preventDefault()}>
        {label}
      </label>
    </div>);
};
