...
const Input = ({ value: initialValue, onChange, placeholder = '', disabled = false, type, label, id, name, className, size = InputSize.MD, isRequired, rounded, roundedFull, showIcon = false, customIconSVG, customIconName, }) => {
    ...
    return (<div className="relative w-full">
      {label && (<Label className="mb-2 block text-sm text-atom-input-text" htmlFor={name}>
          {label}
          {isRequired && <span className="text-error"> *</span>}
        </Label>)}
      <div className="relative">
        <input title={name} type={inputType} id={id} name={name} value={value} onChange={handleChange} placeholder={placeholder} disabled={disabled} className={`${inputClass} ${disabled ? 'input-disabled' : ''} ${error ? 'input-error' : ''}`} required={isRequired}/>
        {showIcon && (<span className="absolute inset-y-0 right-0 flex cursor-pointer items-center pr-3" onClick={() => {
            ...
            {customIconSVG ? (<Icon name={''} variant={'outline'}>
                {customIconSVG}
              </Icon>) : (<Icon name={iconName} variant={'outline'}/>)}
          </span>)}
      </div>
